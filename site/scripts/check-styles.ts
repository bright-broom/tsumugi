/** Keep static presentation in styles/, allowing only data-driven CSS variables in JSX. */
  ...
  function visit(node: ts.Node) {
    if (ts.isJsxAttribute(node) && node.name.getText(source) === 'style') {
      ...
      if (
        ...
        problems.push(
          `${filename}:${line}: static style belongs in styles/ (only CSS variables are allowed)`,
        );
      }
    }
    if (
      ...
      problems.push(`${filename}: inline <style> belongs in styles/`);
    }
  ...
  function visit(dir: string) {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) visit(file);
        ...
        problems.push(`${relative(root, file)}: styles must be centralized in styles/`);
...
if (resolve(process.argv[1] ?? '') === fileURLToPath(import.meta.url)) {
  const problems = checkStyleSources(join(import.meta.dirname, '..'));
  if (problems.length) {
