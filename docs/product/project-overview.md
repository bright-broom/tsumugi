...
> **開発を引き継ぐとき（人・AI）は、まず [AGENTS.md](../../AGENTS.md) と [docs/status.md](../status.md) を読む。**
...
最新の構成と起動手順は [ルートREADME](../../README.md)、実装規約は [開発ガイド](../development.md) を参照します。アプリはルート直下の `src/`、公開素材は `public/`、補助設定は `config/`、生成・検査は `tools/`、仕様・事業資料は `docs/` に分けています。
...
flowchart TD
  subgraph edit["人が編集する正本"]
    direction LR
    tokens["src/styles/<br/>design.tokens.json"]
    css["src/styles/globals.css<br/>Tailwind の中央管理"]
...
- **金額の正本は `src/content/prices.ts` の1か所。** ページも検査も OGP 画像もここを読む。`docs/business/紬_事業の中身.xlsx` は写しで、Excel を直してもサイトは変わらない
- **CSS の正本は `src/styles/`。** `globals.css` が入口となり、Tailwindが共通テーマと各部品の指定を `public/theme.css` にコンパイルする
- ビルドの4段階と検査の中身は [docs/development.md](../development.md) に図がある
...
詳しくは [ADR 0001](../architecture/0001-pages-router.md)。なお、開発サーバーの HTML にはホットリロード用の JS が入る。**0バイトの対象は `out/`（納品物）。**

```mermaid
flowchart LR
  code["src/"] --> dev["npm run dev"]
...
WARN 1 は `PLACEHOLDER=true`（電話番号・住所が仮）。公開前に潰す既知の1件（[公開前にやること](../operations.md)）。
ほかのコマンド（`tokens`・`og`・検査のオプション）は [docs/development.md](../development.md)。
...
- 手順の詳細は [docs/operations.md](../operations.md)、層を分けた理由は [ADR 0004](../architecture/0004-content-and-routing.md)
...
flowchart LR
  ...
  q3["コマンドと<br/>決まりごとは？"] --> d3["docs/development.md"]
...
| 文書 | 中身 |
|---|---|
| [docs/status.md](../status.md) | 現状と残課題（作業を終えたら更新する） |
| [AGENTS.md](../../AGENTS.md) | 崩してはいけないこと・合格ライン・進め方・はまりどころ |
| [docs/development.md](../development.md) | コマンド・ビルドと検査の流れ・書き方と置き場所の決まり |
| [docs/architecture/](../architecture/README.md) | 技術的な判断の一覧と ADR |
| [docs/spec.md](../spec.md) | 検査している項目と、検査で見つかった不具合 |
| [docs/product/](../README.md) | サイトの主張・値付け・デザイン |
| [docs/business/](../business/紬_ビジネスガイドライン.md) | 事業の規範と数字（正本） |
| [docs/operations.md](../operations.md) | 公開前にやること・顧客サイトの作り方・デプロイ |
| [docs/history/](../history/2026-09-migration.md) | Python／Astro から Next.js への移行の記録 |
