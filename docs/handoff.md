...
`npm run verify`（ブラウザ実測）・`npm run og` は、検査用のローカルサーバーを立てる。**サンドボックスがローカルの待受を禁じている環境では実行できない**（`listen EPERM 127.0.0.1`）。同じ環境では次も使えないことがある。

| 症状                                                    | 意味                                                                    |
| ------------------------------------------------------- | ----------------------------------------------------------------------- |
| `listen EPERM 127.0.0.1`                                | 検査用サーバーを立てられない。`verify` のブラウザ部分と `og` が動かない |
| Playwright が `icudtl.dat not found` で落ちる           | ブラウザ本体（`~/Library/Caches/ms-playwright`）を読めない              |
| `gh` が `open …/gh/config.yml: operation not permitted` | CI のログを取得できない                                                 |
| `fetch failed`（外部 URL）                              | プロキシ経由でしか出られない。`NODE_USE_ENV_PROXY=1` を付ける           |

**オーナーが許可すれば動く。** 設定（`~/.claude/settings.json` か `.claude/settings.local.json`）に次を入れる。設定の変更はオーナーの判断で、AI が勝手に書き換えない。

```json
{
  "sandbox": {
    "network": { "allowLocalBinding": true },
    "filesystem": { "allowRead": ["~/Library/Caches/ms-playwright"] }
  }
}
```

**許可が無いときの代わりの確かめ方。** 検査の中身は `tools/verify/in-page.ts`（`CONTRAST` `TAP` `SMALL_TEXT` `ICON_RATIO` `OVERFLOW` `FIG_TEXT` `FIG_CONTRAST`）と `tools/verify/thresholds.ts`（44px・14px・1.10〜1.30 倍・AA 4.5/3.0・390px）に置いてある。**同じ関数と同じしきい値**を、手元で動くブラウザ（エージェントのブラウザ機能など）から開発サーバーか `file://` の `out/` に対して流せば、同じ判定が取れる。全ページ分を回し、件数と失敗の内訳を記録する。

この代替で**確かめられないもの**は必ず未確認として残す：LCP（実測値）、コンソールエラー、`npm run test:security-browser`（CSP のブラウザ確認）、共有カードの再生成。変更前後を比べるときは、`origin/main` を別 worktree に出して `npm run build` し（`node_modules` は `cp -cR` で複製できる）、同じ方法で両方を測って**差が出たかどうか**で判断する。測り方の環境差（数値の絶対値）を退行と取り違えない。
