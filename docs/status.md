...
- 最終更新：2026-09-17（共通フッター改修とヒーロー画像上部の切れを修正・検証。Vercel 本番は商用プラン変更待ち）
...
| 検査 | 本番全項目 **PASS 581 / WARN 3 / FAIL 0**、静的プレビュー PASS 318 / WARN 1 / FAIL 0（対象なし 40 件）。本番 WARN は専門家確認 2 件・受入確認記録の未実施（ADR 0056）。TOP の重複セクション整理によりブラウザの検査対象が 5 件減少（ADR 0057）。 |
| 型・依存・文言 | `npm run check` が通る。ESLint エラー・警告 0、Vitest 518 件・料金モデル 12 件合格 |
...
| 暫定ヒーロー | ヘッダー直下に国生みの画像を配置。文字のない自己完結SVG（WebP内包、約635 KiB）と、i18nから描画するHTMLのコピーに分離。すべての幅でコピーを絵の中に重ね、画像は上端基準で人物の頭を保持。7 種類の幅で検証（ADR 0019・0029・[0060](architecture/0060-hero-top-alignment.md)） |
