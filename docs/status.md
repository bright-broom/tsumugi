...
- 最終更新：2026-09-16（Issue 一斉対応の統合。ブラウザ実測を含む verify は未実行）
...
- **ブラウザ実測を含む `npm run verify` は未実行。** 作業環境のサンドボックスがローカルのポートの待ち受けを禁じていたため。マージ前に手元か CI で全項目を回し、`AGENTS.md`・`docs/spec.md`・この文書の件数（602 など）を更新する。
...
- #19〜#22（店舗情報・CTA・スタッフ・お客様の声）は、ブランチ `claude/template-store-cta` に途中の WIP コミットがあるだけで、型検査が通らないため統合していない。
