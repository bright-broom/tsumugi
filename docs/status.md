...
- 最終更新：2026-09-14（生成り・墨・琥珀の3色リデザイン）
...
| 共通フッター | 生成りの面と琥珀のCTAに、相談・連絡先・案内・大きなブランド文字を配置。アイコン32個、20ページへの案内、先頭への移動、現在地を表示（ADR 0008） |
| ホーム本文 | 生成り70%・墨20%・琥珀10%をUI面積の目安として、広い図解・左右に分けた説明・料金比較・FAQを再構成。元の図解4点と全説明文を保持、本文のLucideアイコンは31→45。本文の構成はADR 0007、現行の配色はADR 0014 |
| 共通ヘッダー | 生成り・墨・琥珀・明朝の屋号。高さ88/72px、問い合わせCTAとネイティブの全体メニュー。追加の実行時JS・外部フォントなし（ADR 0006） |
