...
- 最終更新：2026-09-14（共通部品と全ページの表示を整理）
...
| 型・依存・文言 | `npm run check` が通る。ESLint エラー・警告 0、Vitest 114 件合格 |
...
| ホーム本文 | アイボリー70%・エスプレッソ20%・琥珀10%をUI面積の目安として、広い図解・左右に分けた説明・料金比較・FAQを再構成。図解4点と本文のLucideアイコン45個を保持。所有・FAQの説明は共通化（ADR 0017）。本文の構成はADR 0007、現行の配色はADR 0014 |
