...
- 最終更新：2026-09-14（Tailwind CSSへの中央管理移行）
...
| 検査 | `npm run verify`：**PASS 608 / WARN 1 / FAIL 0**。WARN は `PLACEHOLDER=true` による1件。`npm run verify -- --static`：PASS 360 / WARN 1 / FAIL 0 |
| 型・依存・文言 | `npm run check` が通る。ESLint エラー・警告 0、Vitest 120 件合格 |
...
2026-09-14に、ユーザー指定の月140時間・顧客層未定を前提として [料金再設計案](business/pricing-redesign-140h.md) と [計算モデル](../tools/pricing/README.md) を作成した。制作3商品と任意の継続支援、変更枠、増築時の充当、現金回収、工数と解約率の感度を整理。新料金は経営判断用の提案であり、公開価格・既存契約には適用していない。

料金案を採用する際は、実案件の工数・顧客獲得費を検証し、`src/content/prices.ts` と全関連文言・図・契約・ビジネス文書を同期する。CMS等を含む商品は提供能力を確認してから受付開始する。本人の時間を含む月次原価と契約可能な84時間の上限は、`node --test tools/pricing/model.test.mjs` の9件で検証済み（ADR 0010）。
