...
**このリポジトリと、その週次の成果物に置いてはいけないもの**：CMS のデータ、問い合わせの内容、顧客の非公開の素材、秘密情報。週次の成果物は、リポジトリを読める人なら取得できる（2026-09-18 の確認ではリポジトリは Private。ADR 0070）。これらの保管先はオーナーが別に決める（「保管先の記録」）。
...
- 浅い clone（`git clone --depth` や GitHub Actions の既定のチェックアウト）ではバックアップは作られない。親のコミットを欠いたバンドルは復元の clone で失敗するため、`git fetch --unshallow`（Actions では `fetch-depth: 0`）で履歴をそろえてから実行する
...
### 業務データ（`.data/`）の暗号化バックアップ

社内ツールの実データ（顧客・案件・見積・請求・入金・振込先の設定。`.data/`、git 管理外）は、上の週次のバックアップに入らない。`npm run backup:private` で 1 つの暗号化ファイルにまとめ、オーナーが決めた保管先（外付けディスク・オーナー名義のクラウドなど）へ写す（[ADR 0088](architecture/0088-private-data-backup.md)）。

```bash
export TSUMUGI_BACKUP_PASSPHRASE='…'   # 12 文字以上。引数には書かない（シェルの履歴に残るため）
npm run backup:private -- create  --out <保管先のフォルダ>            # tsumugi-data-<日時>.tsmg を作り、読み戻して照合する
npm run backup:private -- verify  --file <バックアップ>               # 復号して全ファイルの SHA-256 を照合する（書き出さない）
npm run backup:private -- restore --file <バックアップ> --to <空のフォルダ>
npm run backup:private -- prune   --out <保管先のフォルダ> --keep 12 # 新しい 12 世代を残して古いものを消す
unset TSUMUGI_BACKUP_PASSPHRASE
```

- 暗号は AES-256-GCM、鍵はパスフレーズから scrypt で作る。**パスフレーズをなくすと戻せない。** パスワード管理ツールに保管し、2 人目も取り出せるようにする。リポジトリ・チャット・Issue には書かない。
- 作成中は社内ツールと同じロックを取る（ADR 0072）。他の社内操作の実行中や、前回のロックが残っているときは作らずに止まる。
- 取り出しは、空の（またはまだない）フォルダにだけ行う。今ある `.data/` を上書きしない。戻すときは、今の `.data/` を別名に退避してから、空の `.data/` へ取り出す。
- 世代は 12 週を目安に `prune` で整理する。プライバシーの説明で約束した削除（契約に至らなかった方の情報は 1 年）を、バックアップでも守るため。
- 保管先を `.data/` の中にはできない（次のバックアップが前のものを含んで膨らむ）。

### 保管先の記録

| データ                                 | 保管先                       | 公開の有無・暗号化                                          | 保持期間 | 決めた日・決めた人         |
| -------------------------------------- | ---------------------------- | ----------------------------------------------------------- | -------- | -------------------------- |
| リポジトリ（ソース・素材・文書・設定） | GitHub の週次成果物（90 日） | リポジトリを読める人（2026-09-18 時点は Private。ADR 0070） | 90 日    | 2026-09-16（ADR 0045、仮） |
| リポジトリの独立した写し               |                              |                                                             |          |                            |
| 社内ツールの実データ（`.data/`）       |                              |                                                             |          |                            |
| CMS のデータ                           |                              |                                                             |          |                            |
| 問い合わせのデータ                     |                              |                                                             |          |                            |
...
### 自動で見るもの（cron は毎時。実際は 3〜6 時間おき）

GitHub Actions の schedule は混雑時に遅れ・間引かれるため、毎時の指定でも実行は 3〜6 時間おきになる（2026-09-19〜26 の実測で 2.4〜6.3 時間）。障害に気づくまでの時間はこの間隔が目安。短くしたいときは GitHub の外の監視で補う（ADR 0083）。
...
| 不存在 URL                   | —                        | 404 にならない                                                                       |
| 参照 CSS・画像・書体         | —                        | 直接参照と CSS の url()・@import が指す同一サイトのファイルが 200 でない・種類が違う |
...
| 項目       | FAIL                                                                                                                                               |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| 監視の実行 | 直近の完了した実行が 12 時間より古い・一度も実行されていない（GitHub の schedule は毎時指定でも 3〜6 時間おきにしか動かないため。ADR 0083 の追記） |
| 監視の中身 | 手順「Monitor」が飛ばされている・実行に含まれていない                                                                                              |
| 直近の結果 | success でない                                                                                                                                     |
| 連続の失敗 | 2 回以上続けて失敗している                                                                                                                         |

FAIL があると `site-monitor-down` ラベルの issue が 1 件だけ立つ（既にあれば追記）。見張りで FAIL がなくなれば（失敗のあとの成功、止まっていた監視の再開のどちらも）同じ issue が閉じられる。issue はリポジトリを見ている全員に届くので、失敗通知が cron を最後に変更した 1 人にしか届かない問題を補う。

**この見張りも GitHub Actions の上で動く。** GitHub 側の停止や、ワークフローごと止められた場合は検出できない。月に一度の手動確認（下の 6 番）は続ける。

### 日付で変わる案内の反映（毎日 0:10）

臨時休業・臨時の営業時間は、**ビルドした日**を基準に表示している。静的なサイトは置いたままでは変わらないので、境目の日に作り直す（[ADR 0084](architecture/0084-date-boundary-refresh.md)）。

- `npm run refresh` が、前日から今日までに表示が変わる境目（案内が**始まる日**と、**終わった翌日**）を出す。手元でも `npm run refresh -- --today 2026-12-29` のように試せる。
- ワークフロー Daily refresh が毎日 0:10（日本時間）に実行し、境目のある日だけ Vercel の配備フックを叩いて作り直す。境目が無い日は何もしない。
- **臨時の案内を使うサイトでは、シークレット `VERCEL_DEPLOY_HOOK` を設定する**（Vercel のプロジェクト → Settings → Git → Deploy Hooks で作った URL）。境目の日に未設定だと、その日のジョブが失敗して気づける。
- 反映の完了までは見ていない。作り直しのあと、配信内容が main と一致するかは Release check が見る。
...
5. **監視そのものが止まったとき**（Actions に数時間分の実行がない）：`site-monitor-down` の issue と Monitor watchdog の結果を見る。Actions の画面でワークフローが無効になっていないか確かめ、有効に戻す。動きがない期間が続くと定期実行が止まることがある（GitHub の仕様）
...
| 2 人目への連絡方法                                              |                |            |
| `ALERT_ASSIGNEES`（障害 issue の割り当て先）                    | `bright-broom` | 2026-09-24 |
| `VERCEL_DEPLOY_HOOK`（日付境界の作り直し。値はシークレット）    | 設定済み       | 2026-09-24 |
